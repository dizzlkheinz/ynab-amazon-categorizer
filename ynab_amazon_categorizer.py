...
YNAB_ACCOUNT_ID = os.getenv('YNAB_ACCOUNT_ID')  # Optional: specific account ID or None for all accounts

# If environment variables aren't set, try loading from config file
if not YNAB_API_KEY or not YNAB_BUDGET_ID:
    config_file = 'ynab_config.txt'
        ...
        with open(config_file, 'r') as f:
            for line in f:
                ...
                if line.startswith('YNAB_API_KEY='):
                    ...
                    YNAB_ACCOUNT_ID = account_id_value if account_id_value.lower() != 'none' else None

# Validate required configuration
if not YNAB_API_KEY:
    print("ERROR: YNAB_API_KEY not found!")
    print("Please set it as an environment variable or create a ynab_config.txt file.")
...
print(f"✓ Account ID: {YNAB_ACCOUNT_ID[:8]}..." if YNAB_ACCOUNT_ID else "✓ All accounts")
# --- END CONFIGURATION ---
...
def parse_amazon_orders_page(orders_text):
    ...
    for match in order_matches:
        ...
        if next_order:
            ...
            order_content = orders_text[start_pos:start_pos + 2000]  # Take next 2000 chars
        ...
        for line in lines:
            ...
            if (any(word in line.lower() for word in ['pack', 'count', 'size', 'oz', 'ml', 'lbs', 'kg', 'inch', 'cm']) or
                re.search(r'[A-Z][a-z].*[A-Z]', line) or  # Mixed case indicating product names
                len(line.split()) >= 5):  # Long descriptive lines
                ...
                skip_words = ['account', 'orders', 'cart', 'search', 'hello', 'browse', 'amazon', 'prime', 'shipping']
...
def prompt_for_amazon_orders_data():
    ...
    if parsed_orders:
        ...
        for order in parsed_orders[:3]:
            print(f"  - Order {order['order_id']}: ${order['total']} on {order['date']}")
...
def prompt_for_item_details():
    ...
    # Get price per item
    while True:
        price_input = input("Enter item price (optional, press Enter to skip): ").strip()
        if not price_input:
            break
        try:
...
def get_categories(budget_id):
    ...
    for group in data['category_groups']:
        if group.get('hidden', False) or group.get('id') == internal_master_category_group_id:
            continue
        group_name = group['name']
        for category in group['categories']:
             if (not category.get('hidden', False) and
                 not category.get('deleted', False) and
                 category.get('name') != 'Inflow: Ready to Assign'):
                cat_id = category['id']
        ...
        if text_before_cursor:
            for category_name in self.categories:
                if text_before_cursor in category_name.lower():
                    yield Completion(category_name,start_position=-len(text_before_cursor))
    ...
    while True:
        try:
            user_input = prompt(
                ...
                completer=category_completer, history=history, reserve_space_for_menu=5,
            ).strip()
            if not user_input or user_input.lower() == 'b': return None, None
            ...
            if input_lower in name_to_id_map:
                ...
                for name, cat_id in category_completer.category_list:
                    if cat_id == selected_id: selected_display_name = name; break
                print(f"Selected: {selected_display_name}")
                return selected_id, selected_display_name
            else: print(f"Error: '{user_input}' is not a recognized category. Please use Tab completion or try again.")
        except EOFError: print("\nOperation cancelled by user (EOF)."); return None, None
        except KeyboardInterrupt: print("\nOperation cancelled by user (KeyboardInterrupt)."); return None, None
...
print("You can paste Amazon orders page content to automatically match transactions with order details.")
provide_orders = input("Would you like to provide Amazon orders data? (y/n, default y): ").lower()
...
if provide_orders == 'y':
    ...
    if parsed_orders:
        ...
        for order in parsed_orders[:3]:  # Show first 3
            print(f"  - Order {order['order_id']}: ${order.get('total', 'N/A')} ({len(order.get('items', []))} items)")
        if len(parsed_orders) > 3:
            print(f"  ... and {len(parsed_orders) - 3} more orders")
    else:
...
if YNAB_ACCOUNT_ID:
    transactions_endpoint = f"/budgets/{YNAB_BUDGET_ID}/accounts/{YNAB_ACCOUNT_ID}/transactions"
transactions_data = get_ynab_data(transactions_endpoint)
if transactions_data is None or 'transactions' not in transactions_data: exit()
...
for t in transactions:
    ...
    is_valid_for_processing = ( is_amazon and is_uncategorized and is_not_reconciled and t.get('amount', 0) != 0 and t.get('transfer_account_id') is None and not t.get('subtransactions') and t.get('import_id') is not None )
    if is_valid_for_processing: transactions_to_process.append(t)
...
for i, t in enumerate(transactions_to_process):
    ...
    if amount_milliunits > 0: 
        ...
        process_inflow = input("Process this inflow (refund/credit)? (y/n, default n): ").lower()
        if process_inflow != 'y':
    ...
    if parsed_orders:
        ...
        if matching_order:
            ...
            print(f"     Order Link: {generate_amazon_order_link(matching_order['order_id'])}")
            ...
            print(f"  ⚠ No matching order found in parsed Amazon data")

    while True: # Action loop (c, s, q)
        action = input("Action? (c = categorize/split, s = skip, q = quit, default c): ").lower()
        if not action:
            action = 'c'
        if action == 'q': print("Quitting."); exit()
        elif action == 's': print("Skipping."); break # Next transaction
        elif action == 'c':
            # --- Categorization Logic ---
            updated_payload_dict = None
            
            # Check if there are multiple items and suggest splitting
            if matching_order and matching_order.get('items') and len(matching_order['items']) > 1:
            ...
            if split_decision != 'y':
                ...
                if category_id is None: continue # Back to action prompt
                ...
                if matching_order:
                    print(f"Using matched order data for memo generation...")
                    item_details = {
                    ...
                    manual_entry = input("No order match found. Enter item details manually? (y/n, default n): ").lower()
                    if manual_entry == 'y':
                ...
                if item_details:
                    if isinstance(item_details, dict) and 'items' in item_details:
                        ...
                        items_text = item_details['items'][0] if item_details['items'] else "Amazon Purchase"
                        order_link = generate_amazon_order_link(item_details['order_id'])
                        enhanced_memo = f"{items_text}\n {order_link}" if order_link else items_text
                    else:
                        # Manual item details
                        enhanced_memo = generate_enhanced_memo(original_memo, None, item_details)
                else:
                    # No item details
                ...
                update_payload = {
                    ...
                    "memo": memo_input if memo_input else original_memo, # Use new memo or keep original
                ...
                while remaining_milliunits != 0:
                    ...
                    if matching_order and matching_order.get('items'):
                        ...
                        if split_count <= len(items):
                            ...
                            print(f"Additional split for remaining items")
                    ...
                    while True:
                        try:
                            ...
                            amount_str = input(f"Enter amount for '{category_name}' (positive, max {max_amount:.2f}, default {max_amount:.2f}): ")
                            if not amount_str:
                                amount_str = str(max_amount)
                            # ... (amount validation and calculation same as v3) ...
                            split_amount_float = float(amount_str)
                            if split_amount_float <= 0: print("Amount must be positive."); continue
                            split_amount_milliunits = int(round(split_amount_float * 1000))
                            if split_amount_milliunits > abs(remaining_milliunits) + 1: print(f"Amount exceeds remaining. Max {abs(remaining_milliunits / 1000.0):.2f}"); continue
                            split_amount_milliunits = -abs(split_amount_milliunits) # Ensure negative for outflow
                            if abs(split_amount_milliunits - remaining_milliunits) <= 1 : # Equal or very close
                                print("Amount covers remaining balance.")
                                split_amount_milliunits = remaining_milliunits # Assign exact remainder
                            elif split_amount_milliunits > abs(remaining_milliunits): continue # Should be caught, but safety
                            break # Amount valid
                    ...
                    if matching_order:
                        print(f"Using matched order data for split memo...")
                        items = matching_order.get('items', [])
                        if split_count <= len(items):
                            # Use the specific item for this split
                            items_text = items[split_count - 1]
                            order_link = generate_amazon_order_link(matching_order['order_id'])
                            suggested_split_memo = f"{items_text}\n {order_link}" if order_link else items_text
                        else:
                        ...
                        manual_entry = input("Enter item details for this split? (y/n, default n): ").lower()
                        if manual_entry == 'y':
                            item_details = prompt_for_item_details()
                            if item_details:
                                suggested_split_memo = generate_enhanced_memo("", None, item_details)
                ...
                if remaining_milliunits == 0 and subtransactions:
                    update_payload = {
                        ...
                        "memo": generate_split_summary_memo(matching_order) if matching_order else original_memo, # Generate summary memo for splits
                        "cleared": t.get('cleared'),
                        "approved": True,
                        "flag_color": t.get('flag_color'),
                        "import_id": t.get('import_id'),
            ...
            if updated_payload_dict:
                ...
                if preview_dict.get('category_id'):
                    category_name = category_id_map.get(preview_dict['category_id'], 'Unknown Category')
                ...
                if preview_dict.get('subtransactions'):
                    for subtrans in preview_dict['subtransactions']:
                        if subtrans.get('category_id'):
                            cat_name = category_id_map.get(subtrans['category_id'], 'Unknown Category')
                ...
                if confirm == 'y':
                    ...
                    else: print("Update failed."); continue # Back to action prompt
                else: print("Update cancelled."); continue # Back to action prompt
            elif action == 'c': continue # Back to action prompt if categorization started but didn't complete
            else: continue # Back to action prompt
